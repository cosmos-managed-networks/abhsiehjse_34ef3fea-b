...
HTTPS_GIT := github.com/avnifenf/abhsiehjse.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=abhsiehjse \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=newabhishekndjwf \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/newabhishekndjwf ./cmd/newabhishekndjwf
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/newabhishekndjwf.exe ./cmd/newabhishekndjwf
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/newabhishekndjwf
...
draw-deps:
	...
	@goviz -i ./cmd/newabhishekndjwf -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c abhsiehjse --local -f chains.yaml
endif
...
set-testnet-configs:
	newabhishekndjwf config set client chain-id chainid-1
	newabhishekndjwf config set client keyring-backend test
	newabhishekndjwf config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | newabhishekndjwf keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | newabhishekndjwf keys add acc1 --recover`
